...
export default function ContactPageContent() {
  ...
  return (
    <main className="relative z-10 mx-auto max-w-4xl px-6 pt-32 pb-24">
      ...
      <p className="mt-4 text-text-secondary">{c.subtitle}</p>

      <div className="mt-12 grid gap-8 md:grid-cols-2">
        {/* Contact Info */}
        <div className="rounded-2xl border border-border bg-bg-card p-8 card-glow">
          <h2 className="text-lg font-semibold text-white">{c.getInTouch}</h2>
          <div className="mt-6 space-y-6 text-[15px] text-text-secondary">
            <div>
              <p className="text-sm text-text-tertiary">{c.ceoLabel}</p>
              <p className="mt-1 text-white">{c.ceoName}</p>
            </div>
            <div>
              <p className="text-sm text-text-tertiary">{c.emailLabel}</p>
              <a
        ...
        {/* Contact Form */}
        <div className="rounded-2xl border border-border bg-bg-card p-8 card-glow">
          <h2 className="text-lg font-semibold text-white">{c.sendMessage}</h2>
          <form
